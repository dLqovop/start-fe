const todayPhoto = [
  ...
  function printItem(item) {
    const text = `
    <div class="con" onclick="location.href='${item.url}';">
      <div class="image" style="margin: 5px;">
        <img src="${item.img}" width="100">
      </div> 
      <div style="font-size:1.5px; width:100px;">${item.title}</div>
    </div>
      `;
    $wrap.innerHTML += text;
  }
  ...
  function printPage(page) {
    let startIndex = (page - 1) * listSize;

    if(startIndex < 0){
        startIndex=0;
    }
    for (let i = startIndex; i < listSize * page; i++) {
        if(todayPhoto[i]){ //배열에 없는 이미지 출력 오류 제거
